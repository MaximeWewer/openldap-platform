...
`uid` and `cn` must be unique across OUs - every verb but the initial create resolves an entry by searching from the base DN, and the per-user Secret is keyed on the uid alone. Changing `ou:` on an existing entry is governed by `onOuChange`: `warn` (default) reconciles it where it stands, `move` relocates it with a modrdn that keeps its entryUUID and password hash and re-points the `olcAccess` rules and group `member` values naming the old DN, `fail` stops the Job. Membership is reconciled on full DNs, so a `member` left pointing at an old DN is repaired on the next upgrade even without the `refint` overlay. [recipes.md §7](docs/recipes.md#moving-an-entry-to-another-ou) has the trade-offs and the hand procedure.
